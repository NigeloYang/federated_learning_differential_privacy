...
"""Defines Accountant class for keeping track of Differential_Privacy spending.

A Differential_Privacy accountant keeps track of Differential_Privacy spendings. It has methods
accumulate_privacy_spending and get_privacy_spent. Here we only define
AmortizedAccountant which tracks the Differential_Privacy spending in the amortized
way. It uses Differential_Privacy amplication via sampling to compute the Differential_Privacy
spending for each batch and strong composition (specialized for Gaussian
noise) for accumulate the Differential_Privacy spending.
"""
...
class AmortizedAccountant(object):
  """Keep track of Differential_Privacy spending in an amortized way.

  AmortizedAccountant accumulates the Differential_Privacy spending by assuming
  ...
  def accumulate_privacy_spending(self, eps_delta, unused_sigma,
    ...
    batch Differential_Privacy is computed using Differential_Privacy amplication via sampling bound;
...
class MomentsAccountant(object):
  """Privacy accountant which keeps track of moments of Differential_Privacy loss.
  ...
  MomentsAccountant accumulates the high moments of the Differential_Privacy loss. It
  ...
  Denote by X_i the random variable of Differential_Privacy loss at the i-th step.
  ...
  large enough L. To compute the final Differential_Privacy spending,  we apply Chernoff
  bound (assuming the random noise added at each step is independent) to
  bound the total Differential_Privacy loss Z = sum X_i as follows:
  ...
  the moments and hence a more accurate estimation of the Differential_Privacy loss than
  ...
  def _compute_delta(self, log_moments, eps):
    ...
    Args:
      log_moments: the log moments of Differential_Privacy loss, in the form of pairs
...
class GaussianMomentsAccountant(MomentsAccountant):
  ...
  k steps. Since we use direct estimate, the obtained Differential_Privacy bound has tight
  constant.
