...
def train(args, model, device, train_loader, optimizer, privacy_engine, epoch):
    ...
    if not args.disable_dp:
        ...
        print(
            f'Train Epoch: {epoch} \t Loss: {np.mean(losses):.6f} \t (ε: {epsilon:.3f}, δ:{args.delta} for α:{best_alpha})'
        )
    else:
    ...
    for _ in range(args.n_runs):
        ...
        if not args.disable_dp:
            ...
            model, optimizer, train_loader = privacy_engine.make_private(
                module=model, optimizer=optimizer, data_loader=train_loader, noise_multiplier=args.sigma,
        ...
        print(f"major parameter:\n"
              f" mnist_{args.lr}_{args.sigma}_{args.max_per_sample_grad_norm}_{args.batch_size}_{args.epochs}"
              )
        
        # repro_str = (f"mnist_{args.lr}_{args.sigma}_{args.max_per_sample_grad_norm}_{args.batch_size}_{args.epochs}")
