...
import random
import numpy as np
from utils.encoding_he import EncodedNumber
from utils.utils_he import invert, powmod, getprimeover, isqrt
...
if __name__ == '__main__':
    ...
    x1 = 90000.23
    y1 = 90
    print('original x1 + y1 = ', x1 + y1 * 0.5)
    x = pubkey.encrypt(x1)
    y = pubkey.encrypt(y1)
    t1 = x + y * 0.5
    print('crypt result x1 + y1= ', privkey.decrypt(t1))

    print('\n array test')
    t2 = np.array([1.0, 2.0], dtype=float)
    t3 = np.array([1.0, 2.0], dtype=float)
    print('original t2 + t3 = ', t2 + t3)
    # crypt array
    ct2 = []
    ct3 = []
    for i in t2:
        tx0 = pubkey.encrypt(i)
        ct2.append(tx0)
    for i in t3:
        tx0 = pubkey.encrypt(i)
        ct3.append(tx0)
    ...
    res = np.add(ct2, ct3)
    print(res)
    d_res = [privkey.decrypt(i) for i in res]
    print('crypt ct2 + ct3 = ', d_res)
