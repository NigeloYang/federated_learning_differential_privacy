...
class KerasModelWrapper(object):
  def __init__(self, keras_model, input_spec, loss):
    ...
    Args:
      ...
      input_spec: Metadata of dataset that desribes the input tensors, which
        will be converted to `tff.Type` specifying the expected type of input and output of the model.
  ...
  def forward_pass(self, batch_input, training=True):
    ...
    Args:
      batch_input: A `collections.Mapping` with two keys, `x` for inputs and `y` for labels.
  ...
  def from_weights(self, model_weights):
    tff.utils.assign(self.keras_model.trainable_variables, list(model_weights.trainable))
    tff.utils.assign(self.keras_model.non_trainable_variables, list(model_weights.non_trainable))
...
def server_update(model, server_optimizer, server_state, weights_delta):
  ...
  grads_and_vars = tf.nest.map_structure(
    lambda x, v: (-1.0 * x, v), tf.nest.flatten(weights_delta), tf.nest.flatten(model_weights.trainable)
  )
...
def build_server_broadcast_message(server_state):
  ...
  Returns:
    A `BroadcastMessage`.
  """
  return BroadcastMessage(model_weights=server_state.model_weights, round_num=server_state.round_num)
