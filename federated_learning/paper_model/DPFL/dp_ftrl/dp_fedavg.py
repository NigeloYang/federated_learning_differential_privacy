...
def build_dpftrl_fedavg_process(
  ...
  """Builds an iterative process that performs federated averaging with differential differential_privacy.
  ...
  Args:
    ...
    clip_norm: The l2 clip norm of client delta for differential differential_privacy. Must
      be positive when `noise_multiplier` is not `None`.
    noise_multiplier: The noise multiplier for differential differential_privacy. The noise
      std for the sum of client deltas is equal to `clip_norm*noise_multiplier`.
      If `None`, no differential differential_privacy mechanism is applied.
