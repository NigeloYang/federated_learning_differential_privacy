...
# 用tf.Variable()标记参数可训练,使用seed使每次生成的随机数相同
...
for epoch in range(epoch):  # 数据集级别的循环，每个epoch循环一次数据集
  for step, (x_train, y_train) in enumerate(train_db):  # batch级别的循环 ，每个step循环一个batch
    with tf.GradientTape() as tape:  # with结构记录梯度信息
      ...
      y_onehot = tf.one_hot(y_train, depth=3)  # 将标签值转换为独热码格式，方便计算loss和accuracy
      loss = tf.reduce_mean(tf.square(y_onehot - y))  # 采用均方误差损失函数 mse = mean(sum(y-out)^2)
